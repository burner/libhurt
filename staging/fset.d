...
public class FSet(T) : ISR!(T) {
	...
	public override ISRIterator!(T) searchIt(T key) {
		...
		static if(is(T : Object)) {
			binarySearch!(T)(this.data, key, null, found, idx);
		} else {
			binarySearch!(T)(this.data, key, T.init, found, idx);
		}
...
unittest {
	...
	foreach(size_t idx, int[] it; lot) {
		...
		foreach(size_t jdx, int jt; it) {
			...
			auto fsetdup = new FSet!(int)(fset);
			assert(fset == fsetdup);
...
"shed","shot","slit","thought","wound"];
	...
	int fillRate = 3;
	FSet!(string) ssset = new FSet!(string)();
	...
	while(ssset.getSize() < (words.length/fillRate)) {
		...
		auto copy = new FSet!(string)(ssset);
		assert(copy == ssset);
	}
