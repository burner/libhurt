...
class GregorianBased : Gregorian {
	...
	public override const Time toTime(uint year, uint month, uint day, 
			uint hour, uint minute, uint second, uint millisecond, uint era) {
		year = getGregorianYear(year, era);
		return super.toTime(year, month, day, hour, minute, second, 
			millisecond, era);
	}
...
package struct EraRange {
	...
	private static void initialize() {
		if(!initialized_) {
			...
			eraRanges[Gregorian.JAPAN] ~= EraRange(4, getTicks(1989, 1, 8), 
				1988, 1, Gregorian.MAX_YEAR);
			eraRanges[Gregorian.JAPAN] ~= EraRange(3, getTicks(1926, 12, 25), 
				1925, 1, 1989);
			eraRanges[Gregorian.JAPAN] ~= EraRange(2, getTicks(1912, 7, 30), 
				1911, 1, 1926);
			eraRanges[Gregorian.JAPAN] ~= EraRange(1, getTicks(1868, 9, 8), 
				1867, 1, 1912);
			eraRanges[Gregorian.TAIWAN] ~= EraRange(1, getTicks(1912, 1, 1), 
				1911, 1, Gregorian.MAX_YEAR);
			eraRanges[Gregorian.KOREA] ~= EraRange(1, getTicks(1, 1, 1), 
				-2333, 2334, Gregorian.MAX_YEAR);
			eraRanges[Gregorian.THAI] ~= EraRange(1, getTicks(1, 1, 1), 
				-543, 544, Gregorian.MAX_YEAR);
	...
	private static EraRange opCall(uint era, long ticks, uint yearOffset, 
			uint minEraYear, uint prevEraYear) {
		EraRange eraRange;
		eraRange.era = era;
