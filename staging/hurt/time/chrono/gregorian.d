...
module hurt.time.chrono.gregorian;

private import hurt.time.chrono.calendar;
...
class Gregorian : Calendar {
	...
	*/
	override const Time toTime(uint year, uint month, uint day, uint hour, 
			uint minute, uint second, uint millisecond, uint era) {
		return Time (getDateTicks(year, month, day, era) + 
			getTimeTicks(hour, minute, second)) + 
			TimeSpan.fromMillis(millisecond);
	}
	...
	*/
	override const uint getDaysInMonth(uint year, uint month, uint era) {
		...
		auto monthDays = isLeapYear(year, era) ? DaysToMonthLeap : 
			DaysToMonthCommon;
	...
	*/
	 ...
	 */
	override const void split(const(Time) time, ref uint year, ref uint month, 
			ref uint day, ref uint doy, ref uint dow, ref uint era) {
	 ...
	 * of months added.
	 */
	override const Time addMonths(const(Time) t, int nMonths, 
	...
	package static void splitDate (long ticks, ref uint year, ref uint month, 
			ref uint day, ref uint dayOfYear, ref uint era) {
		int numDays;
		...
		void calculateYear() {
			...
			year = whole400Years * 400 + whole100Years * 100 + 
				whole4Years * 4 + wholeYears + era;
		...
		auto monthDays = staticIsLeapYear(year, era) ? DaysToMonthLeap : 
			DaysToMonthCommon;
		month = numDays >> 5 + 1;
	...
	package static long getDateTicks (uint year, uint month, uint day, 
			uint era) {
		//
		...
		if(era == BC_ERA) {
			year += 2;
			return -cast(long)( (year - 3) * 365 + year / 4 - year / 100 + 
				year / 400 + monthDays[12] - (monthDays[month - 1] + day - 1)) 
...
unittest {
	...
	//
	t = Gregorian.generic.toTime(2004, 2, 3, 4, 5, 6, 7) + 
		TimeSpan.fromMicros(8);
	d = Gregorian.generic.toDate(t);
	assert(d.year == 2004);
	assert(d.month == 2);
	assert(d.day == 3);
