...
struct Time {
	...
	private enum : long {
		maximum = (TimeSpan.DaysPer400Years * 25 - 366) * 
			TimeSpan.TicksPerDay - 1,
		minimum = -((TimeSpan.DaysPer400Years * 25 - 366) * 
			TimeSpan.TicksPerDay - 1),
	}

	/// Represents the smallest and largest Time value.
	enum Time min = {minimum}, max = {maximum};
...
struct TimeOfDay {
	 ...
	 * --------------
	 */
	static TimeOfDay opCall(uint hours, uint minutes, uint seconds, 
			uint millis=0) {
		TimeOfDay t = void;
		t.hours = hours;
		t.minutes = minutes;
	...
	const TimeSpan span () {
		return TimeSpan.fromHours(hours) + TimeSpan.fromMinutes(minutes) + 
			TimeSpan.fromSeconds(seconds) + TimeSpan.fromMillis(millis);
	}
...
struct Date {
	...
	public uint year;   /// 0 to 9999
	public uint month;  /// 1 .. 12
	public uint dow;    /// 0 .. 6
	public uint doy;    /// 1 .. 366
}
