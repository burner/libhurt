...
module hurt.net.internetaddress;
...
public class InternetAddress : Address
{
		...
		this (const(char)[] host, ushort port = PORT_ANY)
		{
			...
			foreach (int i, char c; host)
			{
				if (c is ':') {
					...
					port = conv!(int,short)(conv!(string,int)(host [i+1 .. $].idup));
					host = host[0 .. i];
					break;
				}
			}
			...
			if(ADDR_NONE == uiaddr) {
				...
				if(!ih.getHostByName(host))
					throw new AddressException(("Unable to resolve " ~ host ~ ":" ~ conv!(int,string)(port)).idup);
		...
		override immutable(char)[] toString()
		{
				...
				return (fromStringz(addr) ~ ":" ~ 
					conv!(int,string)(this.port()));
		}
