	...
	final const size_t produce (Consumer consume)
	{
		...
		if (userinfo_.length || host_.length || port_ != InvalidPort)
		   ...
		   if (port_ != InvalidPort && port_ != this.getDefaultPort(scheme_))
		      ...
		      ret += consume (":"), ret += consume (conv!(uint,string)(port_));
		      }
		   }
	...
	***********************************************************************/
	...
	final Uri parse (const(char)[] uri, bool relative = false)
	{
		...
		if (c is ':')
		   ...
		   scheme_ = toLowerCase(scheme_.idup);
		   mark = i + 1;
		   }
	...
	***********************************************************************/
	...
	private void parseAuthority (const(char)[] auth)
	{
		...
		for (size_t i=mark; i < len; ++i)
			...
			port_ = conv!(int,ushort)(conv!(string,int)(auth[i+1 .. len].idup));
			len = i;
			break;
			}
...
/*unittest {
	...
	assert(host == "www.example.com", host);
	assert(port == InvalidPort, conv!(ushort,string)(port));
