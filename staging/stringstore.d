...
struct strptr(T) {
	...
	private size_t storeIdx;
	private size_t blockIdx;
...
"shed","shot","slit","thought","wound"];
	...
	StringStore!(char) store = new StringStore!(char)();
	strptr!(char)[] ptr = new strptr!(char)[words.length];
	//Deque!(strptr!(char)) ptr = new Deque!(strptr!(char))(words.length);
	StopWatch sw;
	sw.start();
	foreach(idx, word; words) {
		sb.pushBack(word);
		//ptr.pushBack(store.pushBack(sb));
		ptr[idx] = store.pushBack(sb);
		sb.clear();
	}
	log("it took %f seconds for %u words", sw.stop(), words.length); 
	foreach(idx, word; words) {
		assert(ptr[idx].toString() == word);
	}

	string[] ptr2 = new string[words.length];
	sw.start();
	foreach(idx, word; words) {
		sb.pushBack(word);
		//ptr.pushBack(store.pushBack(sb));
		ptr2[idx] = sb.getString();
		sb.clear();
	}
	log("it took %f seconds for %u words", sw.stop(), words.length); 
}}
