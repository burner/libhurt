...
public import hurt.time.chrono.gregorian;
...
public struct ExtendedDate {
	...
	*/
	const uint seconds() { 
		return val.time.seconds + ((mask >>> 0) & 1);
	}
	...
	*/
	...
	debug (Tango_ISO8601) private char[] toStr() {
		return Stdout.layout.convert(
			"{:d} and {:d}-{:d2}-{:d2} :: {:d2}:{:d2}:{:d2}.{:d3} " ~
			"and {:d2}, {}", year_, years(*this), months(*this), days(*this),
...
private size_t doIso8601Date(T)(ref T* p, T[] src, ref FullDate fd,
		ubyte expanded, out ubyte separators) {
	if(expanded > 5)
		throw new IllegalArgumentException(
			"ISO8601 :: year expanded by more than 5 digits does not " 
			~ "fit in int");
...
private size_t doIso8601Time(T)(ref T* p, T[] src, ref FullDate fd,
		ubyte separators, out bool bothValid) {
	size_t eaten() { return p - src.ptr; }
	size_t remaining() { return src.length - eaten(); }
	bool done(const(T[]) s) { return .done(eaten(), src.length, p, s); }
	...
	byte getTimeZone() { 
		return .getTimeZone(p, remaining(), fd, separators, &done);
	}
...
public size_t parseDateAndTime(T)(T[] src, ref FullDate fd) {
	...
	if(doIso8601Date(p, src, fd, cast(ubyte)0, sep) &&
...
int dayOfWeek(int year, int month, int day)
	in {
		...
		int result = Gregorian.generic.getDayOfWeek(
...
// another option would be to add time zone fields to DT and have this fill 
// them

byte getTimeZone(T)(ref const(T)* p, size_t len, ref FullDate fd, 
		ubyte separators, bool delegate(const(T[])) done) {
	bool checkColon() { return .checkColon(p, separators); }
	...
	int hour = void;
	if(parseInt(p, min(cast(size_t)2, len-1), hour) != 2 || hour > 12 || 
			(hour == 0 && factor == 1))
		return BAD;
	...
	// as above
	if( factor > 0 && hours(fd) == 0 && mins(fd) == 0 && secs(fd) == 0 
			&& ms(fd) == 0)
		fd.setEndOfDay();
...
void addMonths(ref FullDate d, int n) { 
	// -1 due to initial being 1
	d.val = Gregorian.generic.addMonths(d.val, n-1);
}
...
int years(const(FullDate) d) { 
	return Gregorian.generic.getYear	  (d.val); }
int months(const(FullDate) d) { 
	return Gregorian.generic.getMonth	 (d.val); }
int days(const(FullDate) d) { 
	return Gregorian.generic.getDayOfMonth(d.val); }
int hours(const(FullDate) d) { 
	return d.val.time.hours;	}
