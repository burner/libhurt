...
public class Hijri : Calendar {
	...
	private enum uint[] DAYS_TO_MONTH = [ 0, 30, 59, 89, 118, 148, 177, 207, 
		236, 266, 295, 325, 355 ];
	...
	public override const Time toTime(uint year, uint month, uint day, 
		...
		return Time((daysSinceJan1(year, month, day) - 1) * 
			TimeSpan.TicksPerDay + getTimeTicks(hour, minute, second)) + 
	...
	public override const DayOfWeek getDayOfWeek(const(Time) time) {
		return cast(DayOfWeek) (cast(uint) 
			(time.ticks / TimeSpan.TicksPerDay + 1) % 7);
	}
