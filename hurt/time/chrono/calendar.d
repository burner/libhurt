...
module hurt.time.chrono.calendar;

public import hurt.time.time;
//private import hurt.core.Exception;
...
public abstract class Calendar {
	...
	*/
	 ...
	 * method.
	 */
	void split(const(Time) time, ref uint year, ref uint month, ref uint day, 
			ref uint doy, ref uint dow, ref uint era) const {
	    year = getYear(time);
	    month = getMonth(time);
	...
	*/
	const Time toTime (uint year, uint month, uint day, uint hour, uint minute, 
			uint second, uint millisecond=0) {
		return toTime (year, month, day, hour, minute, second, millisecond, 
			CURRENT_ERA);
	}
	...
	*/
	const Time toTime (const(Date) d, const(TimeOfDay) t) {
		return toTime (d.year, d.month, d.day, t.hours, t.minutes, t.seconds, 
			t.millis, d.era);
	}
	...
	*/
	abstract const Time toTime(uint year, uint month, uint day, uint hour, 
	...
	*/
	const uint getWeekOfYear(const(Time) time, WeekRule rule, 
		...
		switch (rule) {
			...
			case WeekRule.FirstFourDayWeek:
				...
				return getWeekOfYear(toTime(year, month, day, 0, 0, 0, 0), 
					rule, firstDayOfWeek);
			default:
				break;
			}
